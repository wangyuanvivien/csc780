...
import edu.sfsu.easymemo.camera.CameraManager;
import edu.sfsu.easymemo.camera.ShutterButton;
import edu.sfsu.easymemo.language.LanguageCodeHelper;
import edu.sfsu.easymemo.language.TranslateAsyncTask;
 ...
 */
public final class OCRActivity extends FragmentActivity implements SurfaceHolder.Callback, ShutterButton.OnShutterButtonListener {
	private CaptureFragment fragmentMain;
	...
	/****yuan add***/	
		dateText = (TextView)findViewById(R.id.ocr_date);
...
/*****yuan add******/
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_MOVE:
					...
					try {
						...
						if (lastX >= 0) {
							// Adjust the size of the viewfinder rectangle. Check if the touch event occurs in the corner areas first, because the regions
							// overlap.
							if (((currentX >= rect.left - BIG_BUFFER && currentX <= rect.left + BIG_BUFFER) || (lastX >= rect.left - BIG_BUFFER && lastX <= rect.left
									+ BIG_BUFFER))
									&& ((currentY <= rect.top + BIG_BUFFER && currentY >= rect.top - BIG_BUFFER) || (lastY <= rect.top + BIG_BUFFER && lastY >= rect.top
											- BIG_BUFFER))) {
								// Top left corner: adjust both top and left sides
								cameraManager.adjustFramingRect(2 * (lastX - currentX), 2 * (lastY - currentY));
								viewfinderView.removeResultText();
							} else if (((currentX >= rect.right - BIG_BUFFER && currentX <= rect.right + BIG_BUFFER) || (lastX >= rect.right - BIG_BUFFER && lastX <= rect.right
									+ BIG_BUFFER))
									&& ((currentY <= rect.top + BIG_BUFFER && currentY >= rect.top - BIG_BUFFER) || (lastY <= rect.top + BIG_BUFFER && lastY >= rect.top
											- BIG_BUFFER))) {
								...
								viewfinderView.removeResultText();
							} else if (((currentX >= rect.left - BIG_BUFFER && currentX <= rect.left + BIG_BUFFER) || (lastX >= rect.left - BIG_BUFFER && lastX <= rect.left
									+ BIG_BUFFER))
									&& ((currentY <= rect.bottom + BIG_BUFFER && currentY >= rect.bottom - BIG_BUFFER) || (lastY <= rect.bottom + BIG_BUFFER && lastY >= rect.bottom
											- BIG_BUFFER))) {
								...
								viewfinderView.removeResultText();
							} else if (((currentX >= rect.left - BUFFER && currentX <= rect.left + BUFFER) || (lastX >= rect.left - BUFFER && lastX <= rect.left
									+ BUFFER))
									&& ((currentY <= rect.bottom && currentY >= rect.top) || (lastY <= rect.bottom && lastY >= rect.top))) {
								// Adjusting left side: event falls within BUFFER pixels of left side, and between top and bottom side limits
								cameraManager.adjustFramingRect(2 * (lastX - currentX), 0);
								viewfinderView.removeResultText();
							} else if (((currentX >= rect.right - BUFFER && currentX <= rect.right + BUFFER) || (lastX >= rect.right - BUFFER && lastX <= rect.right
									+ BUFFER))
									&& ((currentY <= rect.bottom && currentY >= rect.top) || (lastY <= rect.bottom && lastY >= rect.top))) {
								// Adjusting right side: event falls within BUFFER pixels of right side, and between top and bottom side limits
								cameraManager.adjustFramingRect(2 * (currentX - lastX), 0);
								viewfinderView.removeResultText();
							} else if (((currentY <= rect.top + BUFFER && currentY >= rect.top - BUFFER) || (lastY <= rect.top + BUFFER && lastY >= rect.top
									- BUFFER))
									&& ((currentX <= rect.right && currentX >= rect.left) || (lastX <= rect.right && lastX >= rect.left))) {
								// Adjusting top side: event falls within BUFFER pixels of top side, and between left and right side limits
								cameraManager.adjustFramingRect(0, 2 * (lastY - currentY));
								viewfinderView.removeResultText();
							} else if (((currentY <= rect.bottom + BUFFER && currentY >= rect.bottom - BUFFER) || (lastY <= rect.bottom + BUFFER && lastY >= rect.bottom
									- BUFFER))
									&& ((currentX <= rect.right && currentX >= rect.left) || (lastX <= rect.right && lastX >= rect.left))) {
		...
		fragmentMain = new CaptureFragment();

		
		/*Bundle data = new Bundle();
		data.putString("date", "20141217");
		FragmentTransaction t = getSupportFragmentManager().beginTransaction();
        fragmentMain.setArguments(data);
        t.add(R.id.fragment_container, fragmentMain);
        t.commit();*/
	...
	public void takePhoto(View v) {
		...
		getSupportFragmentManager().beginTransaction().remove(fragmentMain).commit();
	}
	...
	public void goToCaptureFragment() {
		...
		////////////////////////////////////////////////
		args.putByteArray("lastBitmap", byteArray);
		args.putString("ocrResultTextView", ocrResultTextView.getText().toString());
		String dateOCR = (String) dateText.getText();
		args.putString("date", dateOCR);
	...
	/**
	 * Method to start or restart recognition after the OCR engine has been initialized, or after the app regains focus. Sets state related settings and OCR
	 * engine parameters, and requests camera initialization.
	 */
	void resumeOCR() {
		Log.d(TAG, "resumeOCR()");
	...
	private File getStorageDirectory() {
		...
		if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
			...
			// // "/mnt/sdcard/Android/data/com.crimsonbits.crimsonnote/files/"
	...
	boolean handleOcrDecode(OcrResult ocrResult) {
		...
		ocrResultTextView = (TextView) findViewById(R.id.ocr_result_text_view);
	...
	void handleOcrContinuousDecode(OcrResult ocrResult) {
		...
		viewfinderView.addResultText(new OcrResultText(ocrResult.getText(), ocrResult.getWordConfidences(), ocrResult.getMeanConfidence(), ocrResult
				.getBitmapDimensions(), ocrResult.getRegionBoundingBoxes(), ocrResult.getTextlineBoundingBoxes(), ocrResult.getStripBoundingBoxes(), ocrResult
				.getWordBoundingBoxes(), ocrResult.getCharacterBoundingBoxes()));
		...
		if (CONTINUOUS_DISPLAY_METADATA) {
			...
			statusViewBottom.setText("OCR: " + sourceLanguageReadable + " - Mean confidence: " + meanConfidence.toString() + " - Time required: "
	...
	void handleOcrContinuousDecode(OcrResultFailure obj) {
		...
		if (CONTINUOUS_DISPLAY_METADATA) {
			...
			statusViewBottom.setTextSize(14);
			CharSequence cs = setSpanBetweenTokens("OCR: " + sourceLanguageReadable + " - OCR failed - Time required: " + obj.getTimeRequired() + " ms", "-",
	...
	/**
	 * Given either a Spannable String or a regular String and a token, apply the given CharacterStyle to the span between the tokens.
	 * 
	 * NOTE: This method was adapted from: http://www.androidengineer.com/2010/08/easy-method-for-formatting-android.html
	 * 
	 * <p>
	 * For example, {@code setSpanBetweenTokens("Hello ##world##!", "##", new
	 * ForegroundColorSpan(0xFFFF0000));} will return a CharSequence {@code "Hello world!"} with {@code world} in red.
	 * 
	 */
	...
	/**
	 * Displays an initial message to the user while waiting for the first OCR request to be completed after starting realtime OCR.
	 */
	...
	/**
	 * Requests autofocus after a 350 ms delay. This delay prevents requesting focus when the user just wants to click the shutter button without focusing.
	 * Quick button press/release will trigger onShutterButtonClick() before the focus kicks in.
	 */
	...
	/**
	 * We want the help screen to be shown automatically the first time a new version of the app is run. The easiest way to do this is to check
	 * android:versionCode from the manifest, and compare it to a value stored as a preference.
	 */
	...
	private void retrievePreferences() {
		...
		setSourceLanguage(prefs.getString(PreferencesActivity.KEY_SOURCE_LANGUAGE_PREFERENCE, OCRActivity.DEFAULT_SOURCE_LANGUAGE_CODE));
		setTargetLanguage(prefs.getString(PreferencesActivity.KEY_TARGET_LANGUAGE_PREFERENCE, OCRActivity.DEFAULT_TARGET_LANGUAGE_CODE));
		isTranslationActive = prefs.getBoolean(PreferencesActivity.KEY_TOGGLE_TRANSLATION, false);

		// Retrieve from preferences, and set in this Activity, the capture mode preference
		if (prefs.getBoolean(PreferencesActivity.KEY_CONTINUOUS_PREVIEW, OCRActivity.DEFAULT_TOGGLE_CONTINUOUS)) {
	...
	private void setDefaultPreferences() {
		...
		// Continuous preview
		prefs.edit().putBoolean(PreferencesActivity.KEY_CONTINUOUS_PREVIEW, OCRActivity.DEFAULT_TOGGLE_CONTINUOUS).commit();

		// Recognition language
		prefs.edit().putString(PreferencesActivity.KEY_SOURCE_LANGUAGE_PREFERENCE, OCRActivity.DEFAULT_SOURCE_LANGUAGE_CODE).commit();

		// Translation
		prefs.edit().putBoolean(PreferencesActivity.KEY_TOGGLE_TRANSLATION, OCRActivity.DEFAULT_TOGGLE_TRANSLATION).commit();

		// Translation target language
		prefs.edit().putString(PreferencesActivity.KEY_TARGET_LANGUAGE_PREFERENCE, OCRActivity.DEFAULT_TARGET_LANGUAGE_CODE).commit();

		// Translator
		prefs.edit().putString(PreferencesActivity.KEY_TRANSLATOR, OCRActivity.DEFAULT_TRANSLATOR).commit();

		// OCR Engine
		prefs.edit().putString(PreferencesActivity.KEY_OCR_ENGINE_MODE, OCRActivity.DEFAULT_OCR_ENGINE_MODE).commit();

		// Autofocus
		prefs.edit().putBoolean(PreferencesActivity.KEY_AUTO_FOCUS, OCRActivity.DEFAULT_TOGGLE_AUTO_FOCUS).commit();

		// Disable problematic focus modes
		prefs.edit().putBoolean(PreferencesActivity.KEY_DISABLE_CONTINUOUS_FOCUS, OCRActivity.DEFAULT_DISABLE_CONTINUOUS_FOCUS).commit();

		// Beep
		prefs.edit().putBoolean(PreferencesActivity.KEY_PLAY_BEEP, OCRActivity.DEFAULT_TOGGLE_BEEP).commit();

		// Character blacklist
		prefs.edit().putString(PreferencesActivity.KEY_CHARACTER_BLACKLIST, OcrCharacterHelper.getDefaultBlacklist(OCRActivity.DEFAULT_SOURCE_LANGUAGE_CODE))
				.commit();

		// Character whitelist
		prefs.edit().putString(PreferencesActivity.KEY_CHARACTER_WHITELIST, OcrCharacterHelper.getDefaultWhitelist(OCRActivity.DEFAULT_SOURCE_LANGUAGE_CODE))
				.commit();

		// Page segmentation mode
		prefs.edit().putString(PreferencesActivity.KEY_PAGE_SEGMENTATION_MODE, OCRActivity.DEFAULT_PAGE_SEGMENTATION_MODE).commit();

		// Reversed camera image
		prefs.edit().putBoolean(PreferencesActivity.KEY_REVERSE_IMAGE, OCRActivity.DEFAULT_TOGGLE_REVERSED_IMAGE).commit();

		// Light
		prefs.edit().putBoolean(PreferencesActivity.KEY_TOGGLE_LIGHT, OCRActivity.DEFAULT_TOGGLE_LIGHT).commit();
	}
	...
	void showErrorMessage(String title, String message) {
		new AlertDialog.Builder(this).setTitle(title).setMessage(message).setOnCancelListener(new FinishListener(this))
